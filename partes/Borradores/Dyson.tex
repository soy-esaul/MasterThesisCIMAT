...
\todo[inline]{Unificar notación}

\todo[inline]{Al principio de este capítulo (en el borrador es una sección) escribiría una breve introducción sobre el proceso de Dyson, en qué contexto surge y tal vez explicar un poco la intuición de la forma de la ecuación (esto de repulsión couloumbica) y los casos GUE y GOE. Todo esto para motivar los teoremas.  

Después de eso vendrían los teoremas que tomé de Graczyk y Małecki para aplicarlos a mostrar los resultados de Dyson y de otros proecesos matriciales. También está pendiente incluir algo de texto entre cada teorema, explicando para que se usa cada uno en la tesis.}

\begin{theorem}[First part of Multidimensional Yamada-Watanabe] \label{thm:mult_yamada_watanabe}
    ...
    where $\rho_i:(0,\infty)\to(0,\infty)$ are measurable functions such that 
...
\begin{proof}
    ...
    Applying Theorem \ref{thm:local_zero} we have that the local time of $Y_i - \hat Y_i$ at 0 is 0. Then, we can use the Tanaka's formula to find

    \begin{align*}
        \abs{ Y_i(t) - \hat Y_i(t) } &= \int_0^t \mathrm{sgn}(Y_i - \hat Y_i) ~\d( Y_i(s) - \hat Y_i(s) ),\\
        &= \int_0^t \mathrm{sgn}(Y_i(t) - \hat Y_i(t))(\sigma_i(Y_i) - \sigma_i(\hat Y_i)) ~\d B_i(s)\\
        &\phantom{espacioteeeee}+ \int_0^t \mathrm{sgn}(Y_i(t) - \hat Y_i(t))(b_i(Y_i(t)) - b_i(\hat Y_i(t))) ~ \d t.
    \end{align*}
    ...
    \begin{align*}
        ...
        &= \int_0^t \E{ \abs{ b_i(Y_i(s)) - b_i(\hat Y_i(t)) } }~\d s 
        \le  C\int_0^t \E{\abs{Y_i(t) - \hat Y_i(s)}}~\d s.
    \end{align*}
...
\begin{theorem} \label{thm:dyson_real}
    Let $X_t$ be a symmetric $p\times p$ matrix-valued stochastic process satisfying the stochastic differential equation

    \begin{equation}
        \d X_t = g(X_t) \d B_t h(X_t) + h(X_t) \d B_t^T g(X_t) + b(X_t)\d t, \label{eq:matrix_diffusion}
    \end{equation}

    where $g,h,b$ are real functions acting spectrally, and $X_0$ is a symmetric $p\times p$ matrix with $p$ different eigenvalues. 
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &= \sum_{pqrs} \trans{H}_{ip}H_{qj}\trans{H}_{kr}H_{sm}\bigl(g^2(X)_{pr}h^2(X)_{qs} + g^2(X)_{ps}h^2(X)_{qr} + g^2(X)_{qs}h^2(X)_{pr} + g^2(X)_{qr}h^2(X)_{ps}\bigr)\d t.
    \end{align*}
    ...
    \begin{equation*}
        \d N_{ii} = 2g(\lambda_i)h(\lambda_i)\d W_i + \delta_{ij}\sum_{k\neq j} \frac{ G(\lambda_i,\lambda_k) }{\lambda_i - \lambda_k} \d t.
    \end{equation*}
...
\begin{theorem}
    Let $W_t$ be a complex $p\times p$ Brownian matrix. Suppose that an $\mathcal H_p$ valued stochastic process $X_t$ satisfies the following matrix stochastic differential equation:

    \begin{equation}\label{eq:complex_diff}
        \d X_t = g(X_t)\d  W_t h(X_t) + h(X_t)\d W^*_t g(X_t) + b(X_t)\d t,
    \end{equation}
...
\begin{proof} 
    ...
    \[\d Z \d Z = 0, \qquad \d Z \d \overline Z = 2\d t.\]

    
    Then we can compute the quadratic covariation $\d X_{ij}\d X_{kl}$ using \eqref{eq:complex_diff},

    \begin{align}
        \d X_{ij}\d X_{kl} &=  \d\langle X_{ij},X_{kl} \rangle, \\
        &= \d\langle \left( g(X_t)\d  W_t h(X_t) + h(X_t)\d W^*_t g(X_t) \right)_{ij}, \left(g(X_t)\d W_t h(X_t) + h(X_t)\d W^*_t g(X_t)\right)_{kl} \rangle,\\
        &= \d\langle \left(g(X_t)\d  W_t h(X_t)\right)_{ij}, \left(h(X_t)\d W^*_t g(X_t)\right)_{kl}\rangle + \d\langle \left(g(X_t)\d  W_t h(X_t)\right)_{kl}, \left(h(X_t)\d W^*_t g(X_t)\right)_{ij}\rangle,\\
        &= 2g^2(X_t)_{il}h^2(X_t)_{jk}\d t + 2g^2(X_t)_{kj}h^2(X_t)_{li}\d t,\\
        &= 2\left( g^2(X_t)_{il}h^2(X_t)_{kj} + g^2(X_t)_{jk}h^2(X_t)_{il} \right)\d t.
    \end{align}
...
\begin{theorem} \label{thm:collision}
    Let $\Lambda = (\lambda_i)_{i=1,\dots,p}$ be a process starting at the open simplex $\Delta_p$ and satisfying \eqref{eq:gen_dyson} with functions $g,h,b:\R \to \R$ such that $g^2, h^2, b$ are Lipschitz continuous and $g^2h^2$ is convex or is continuously differentiable with derivative uniformly Lipschitz on $\R$. Then the first collision time $\tau$ defined as in \eqref{eq:collision_time} is infinite a.s.
\end{theorem}
...
\begin{proof}
    ...
    By the Lipschitz conditions on $g^2$ and $h^2$ we find that $\abs{H(x,y,z)} \le 2K^2\abs{(y-x)(z-y)(z-x)}$. Using the last equality, \todo{Escribir detalles de esta cuenta} we can write $2\d A_3 = \d A_4 + \d A_5$, with $0 \le A_4(t) \le K^2 p(p-1)(p-2)t/6$ and

    \begin{align}%Escribir detalles de esta cuenta
...
\todo[inline]{Después de exponer estos teoremas, restaría aplicarlos para exponer primero los resultados de Dyson sobre el proceso de eigenvalores de un movimiento Browniano en el espacio de matrices ortogonales y autoadjuntas, y después aplicarlos para encontrar ecuaciones de procesos de eigenvalores más generales (Wishart, Jacobi, Laguerre). Estos ejemplos vienen en Graczyk y Małecki, así que incluirlos no debería ser tan difícil.}
