...
\end{theorem}
 ...
 \begin{proof}
    ...
    \begin{align*}
        \left(c\partial_z + cz\partial_{zz} + \partial_t\right) [P] &= \sum_{k=0}^{n-2} \left[ c\binom{n}{k}\left( -ct \right)^k \frac{n!}{(n-k-1)!}z^{n-k-1} + cz\binom{n}{k}\left( -ct \right)^k \frac{n!}{(n-k-2)!}z^{n-k-2}\right.\\ &\phantom{espac} + \left.\binom{n}{k+1}(k+1)(-c)^{k+1}t^{k} \frac{n!}{(n-k-1)!}z^{n-k-1}\right] \\
...
Before proceeding with the proper Jacobi process, let us find the expected characteristic polynomial of a static Jacobi matrix. This case is more challenging than the two previous processes, but the static case is greatly simplified by using the tools of Finite Free Probability theory. \todo{Corregir normalización}

%%%% Traducir y corregir normalización
% Let $A$ and $B$ be two $n\times n$ random matrices with independent standard Gaussian entries. The MANOVA matrix $M$ or Jacobi ensemble is defined as

% \begin{equation*}
%     M \coloneqq (A^TA + B^TB)^{-1}A^TA.
% \end{equation*}

% Para obtener $p_M(z)$ el polinomio característico esperado de $M$, podemos primero obtener el de su inversa, $p_{M^{-1}}(z)$. Tenemos que

% \begin{equation*}
%     M^{-1} = (A^TA)^{-1}(A^TA + B^TB) = I + (A^TA)^{-1}B^TB.
% \end{equation*}

% Bajo las hipótesis de que $I$ y $(A^TA)^{-1}B^TB$ son normales y que $(A^TA)^{-1}B^TB$ es invariante bajo conjugación por una matriz ortogonal, se tiene que 

% \begin{equation*}
%     p_{M^{-1}}(z) = p_I(z) \boxplus_n p_{(A^TA)^{-1}B^TB}(z).
% \end{equation*}

% Además, bajo el supuesto de que $(A^TA)^{-1}$ y $B^TB$ son normales e invariantes bajo conjugación por matrices ortogonales, se tiene que

% \begin{equation*}
%     p_{(A^TA)^{-1}B^TB}(z) = p_{(A^TA)^{-1}}\boxtimes p_{B^TB}(z).
% \end{equation*}

% Sabemos además que $p_{B^TB}(z) = L_n(z)$ y $p_{(A^TA)^{-1}}(z)=z^n L_n(1/z)$. Con esto podemos encontrar $p_{M^{-1}}(z)$.
...
% Ahora calculamos $p_{M^{-1}}(z)$, recordando que $p_I(z) = (z-1)^n$
...
% Finalmente, usamos $p_{M}(z) = z^n p_{M^{-1}}(1/z)$ para obtener

% \begin{align*}
%     p_{M}(z) &= z^n \sum_{k=0}^n z^{k-n}(-1)^{n-k}n!\binom{n}{k} \binom{n+k}{k} = \sum_{k=0}^n z^{k}(-1)^{n-k}n!\binom{n}{k} \binom{n+k}{k}.
% \end{align*}

% Esto corresponde al $n$-ésimo polinomio de Legendre evaluado en $1-2z$.

% Resta por probar la invarianza bajo transformaciones ortogonales de las matrices. Es claro que $(A^TA)^{-1}B^TB$ es invariante si $(A^TA)^{-1}$ y $B^TB$ lo son. Recordemos que las matrices de Wishart son invariantes bajo transformaciones ortogonales, entonces resta probar que $(A^TA)^{-1}$ es invariante bajo transformaciones ortogonales, para esto usamos la invarainza de $A^TA$,

% \begin{align*}
%     Q(A^T A)^{-1}Q^* &= (QA^TAQ^*)^{-1} \overset{d}{=} (A^TA)^{-1}.
% \end{align*}

% Con esto se prueba que el polinomio característico esperado de una matriz de Jacobi con parámetros $n,n,n$ es un polinomio de Legendre evaluado en $1-2z$.
% \todo{Traducir y corregir normalización}
%%%% Traducir y corregir normalización 
...
Finally, we use \( p_{M}(z) = z^n p_{M^{-1}}(1/z) \) to obtain
