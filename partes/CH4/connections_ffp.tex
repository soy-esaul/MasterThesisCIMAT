...
\intertext{doing the same procedure for $e^{\sigma_2\partial_x\partial_z}$ we get }
    ...
    \begin{align*}
        ...
        &=\left\{ \left(\sum_{j=0}^{n_2} \binom {n_2}j x^j \partial_x^j \right) \left( \sum_{l=0}^{n_1} \binom {n_1}j x^l \partial_x^l \right) \right\}[x^k],\\
        &= \sum_{j=0}^k \sum_{l\le n-j} \binom{n_2}{j}\binom{n_1}l x^j (x-1)^{n-l} \partial_x^{j+l}[x^k]\\
        &= \sum_{l=0}^k \binom{n_2}{k-l} \binom{n_1}l x^{n-l}(x-1)^l \eqqcolon P_k^{n_2 - k, n_1-k}(2x-1).
    \end{align*}
