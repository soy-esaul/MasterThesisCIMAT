...
\begin{proof}
    ...
    Now we take $C$ to be any $(n-1)\times(n-1)$ orthogonal (or unitary, symplectic, according to $\beta$) matrix such that $C \trans{\vec x} = \norm{\vec x}_2 e_1$, where $e_1 = (1,0,\cdots,0)$ is the first element in the canonical basis of $\R^{n-1}$. Then we have
...
\intertext{doing the same procedure for $e^{\sigma_2\partial_x\partial_z}$ we get }
    ...
    \begin{align*}
        (\partial_t x)(\partial_x \hat p) &+ (\partial_t y)(\partial_y \hat p) + (\partial_t y)(\partial_y \hat p)|_{t=0} \\ 
    ...
    %Notice that by definition, $p(x,y,z)$ is a homogeneus polynomial, and this allows us to 
